...
页面通过 `js/lean-checker.js` 使用 Language Server Protocol。默认配置连接 `wss://live.lean-lang.org/websocket/MathlibDemo`，因此 GitHub Pages 上的静态站点也能提交真实 Mathlib 源码。生产部署若需要独立容量与稳定性，应部署自己的 [lean4web](https://github.com/leanprover-community/lean4web) 服务，然后修改 `data/runtime-config.js` 中的 `leanWebSocket`、`leanProject` 与显示标签。

Lean elaboration 可以执行代码。自建服务必须使用沙箱、网络隔离、资源限制和超时；不要直接把未隔离的 Lean 进程暴露到公网。
